...
var dataDIR = 'C:/Users/hqt2102/OneDrive - cumc.columbia.edu/Stroke/stroke_analysis/javascript/data_js/';
var dataDIRmac = '/Users/Ryan/OneDrive - cumc.columbia.edu/Stroke/stroke_analysis/javascript/data_js/';
dataDIR='./data_js/'; //relative path works for both
var centersDIR = dataDIR + 'MA_n=100_centers.js';
var pointsDIR = dataDIR + 'MA_n=100_points.js';
var linesDIR = dataDIR + 'MA_n=100_lines_RACE_1.js';
var lineScript;
...
function initialize(newMap) {
  ...
  // Json script micmicing Google's example
  // https://developers.google.com/maps/documentation/javascript/earthquakes
  script.src = centersDIR;
  document.getElementsByTagName('head')[0].appendChild(script);

  // // Create a <script> tag and set the JS-json script as soource
  lineScript = document.createElement('script');
  // Json script micmicing Google's example
  // https://developers.google.com/maps/documentation/javascript/earthquakes
  lineScript.src = linesDIR;
  // lineScript.type="module";
  document.getElementsByTagName('head')[0].appendChild(lineScript);

  // Example.test();

  // fetch(linesDIR).then(res => res.json()).then(data => console.log(data))
  centersList={};
  pointsList={};
...
function logKey2(e){
  ...
  linesDIR = dataDIR + "MA_n=100_lines_RACE_"+raceNum+".js";
  console.log(linesDIR);
  lineScript.src = pointsDIR;
  var newMap = false;
  ...
  map = new google.maps.Map(document.getElementById("map"), myOptions);
  initialize(newMap);
  // document.getElementsByTagName('body')[0].appendChild(lineScript);
}
