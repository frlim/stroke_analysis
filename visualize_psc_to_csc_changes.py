...
changed_recs = changed_recs.iloc[:,4]
...
scatter_list.append(
    go.Scattermapbox(lat=points.Latitude,
                 ...
                 marker=dict(size=9,
                             opacity=0.8,
                             color=points.Prop_PSC_to_CSC,
                             colorbar=dict(title="Proportion of Changes from PSC to CSC"),
                             colorscale='plasma'))
    )
...
py.plot(fig, filename=str(data_io.GRAPH_OUTPUT) + '/psc_to_csc_changed_recs_locations.html')