...
changed_recs = pd.read_excel(str(data_io.LOCATION_ANALYSIS_OUTPUT) + '/location_key_changes.xlsx', 
                sheet_name = "Overall", index_col="Locations")
changed_recs = changed_recs.iloc[:,-1]
...
scatter_list.append(
    go.Scattermapbox(lat=points.Latitude,
                 ...
                 marker=dict(size=9,
                             opacity=0.8,
                             color=points.Prop_Changed,
                             colorbar=dict(title="Proportion of Recommendation Changes"),
                             colorscale='plasma'))
    )
...
py.plot(fig, filename=str(data_io.GRAPH_OUTPUT) + '/changed_recs_locations.html')