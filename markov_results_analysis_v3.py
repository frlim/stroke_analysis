    ...
    for race in range(RACE_MIN,RACE_MAX+upper):
        for time_since_symptoms in range(SYMP_MIN,SYMP_MAX+upper,10):
            ...
            for loc in locs:
                ...
                b_markov_res.set_index(['Simulation','Variable'],inplace=True)
                a_markov_res.set_index(['Simulation','Variable'],inplace=True)
                ...
                all_truth = (b_markov_res_centers == basic_res.loc[loc,'BestCenter_be']) | (b_markov_res_centers == basic_res.loc[loc,'BestCenter_af'])
                ...
                all_truth = (a_markov_res_centers == basic_res.loc[loc,'BestCenter_be']) | (a_markov_res_centers == basic_res.loc[loc,'BestCenter_af'])
                ...
                b_columns = [*zip(b_agg.columns.get_level_values(0),b_agg.columns.get_level_values(2))]
                a_columns = [*zip(a_agg.columns.get_level_values(0),a_agg.columns.get_level_values(2))]
                a_agg.columns = pd.MultiIndex.from_tuples(a_columns,names=['strategy','stat'])
                b_agg.columns = pd.MultiIndex.from_tuples(b_columns,names=['strategy','stat'])


                col_names = ['version','strategy','stat']
                out = pd.concat([b_agg,a_agg],axis=1,keys=['beAHA','afAHA'],names=col_names).swaplevel(0,1,axis=1)
                out.sort_index(axis=1,inplace=True)
                # out = pd.concat([out],keys=[str(loc)],names=['location'])
                out.to_excel(data_io.MARKOV_ANALYSIS_OUTPUT/f'times={times_path.stem}_hospitals={hospital_path.stem}_sex={sex_str}_age={age}_race={race}_symptom={time_since_symptoms}_nsim={s_default}_loc={loc}_markov_comparison_v3.xlsx')
